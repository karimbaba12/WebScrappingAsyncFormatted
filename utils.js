...
export const formatValidSectors = (sectorsStr) => {
  ...
  const final = filteredSectors.map((sector) =>
...
export const formatInSectors = (sectorsStr) => {
  ...
  const invalidSectors = sectors.filter(
    (sector) => !validSectors.includes(sector)
  );
  const final = invalidSectors.map((sector) =>
    sector.trim().toLowerCase().replace(/\s+/g, "_").replace(/&/g, "and")
  );
  return final;
};
...
export const formatvalidmajors = (majorStr) => {
  ...
  const processedValidMajors = validmajors.map((major) =>
    major.trim().toLowerCase().replace(/\s+/g, "_").replace(/&/g, "and")
  );
  ...
  const formattedMajors = majors.map((major) =>
    major.toLowerCase().replace(/\s+/g, "_").replace(/&/g, "and")
  );

  // Filter the input majors to find matches in the valid majors
  const filteredMajors = formattedMajors.filter((major) =>
    processedValidMajors.includes(major)
  );
...
export const formatSalary = (salary) => {
  ...
  if (matchA) {
    ...
    ["negotiable", "n/a", "competitive", "to be determined"].includes(
      salary.toLowerCase()
    )
  ) {
    return {
...
export const formatEmploymentPeriod = (periodStr) => {
  const patterns = [
    ...
    {
      pattern: /(\d+)\s*(شهر|أشهر|سنة|سنوات)/,
      handler: (match) => ({ number: parseInt(match[1]), scale: match[2] }),
    },
    // Arabic pattern for "Till the end of"
    {
      pattern: /حتى نهاية (\w+ \d{4})/,
      handler: () => ({ number: 0, scale: "unknown" }),
    },
    // Arabic pattern for specific months
    {
      pattern: /(\d+)\s+أشهر/,
      handler: (match) => ({ number: parseInt(match[1]), scale: "أشهر" }),
    },
    // Arabic pattern for period from date to date
    {
      pattern: /(\d+) (شهر|سنة) من (\d{1,2} \w+ \d{4}) حتى (\d{1,2} \w+ \d{4})/,
      handler: (match) => ({ number: parseInt(match[1]), scale: match[2] }),
    },
    // Arabic pattern for until specific date
    {
      pattern: /حتى (\d{1,2} \w+ \d{4})/,
      handler: () => ({ number: 0, scale: "unknown" }),
    },
    // Arabic pattern for range of dates
    {
      pattern:
        /(\d{1,2}) ([\u0600-\u06FF]+)، (\d{4}) - (\d{1,2}) ([\u0600-\u06FF]+)، (\d{4})/,
      handler: () => ({ number: 0, scale: "unknown" }),
    },
    ...
    {
      pattern:
        /(\d+) (months?|years?) from (\w+ \d{1,2}(st|nd|rd|th)?, \d{4}) till (\w+ \d{1,2}(st|nd|rd|th)?, \d{4})/,
      handler: (match) => ({
        number: parseInt(match[1]),
        scale: match[2].toLowerCase(),
      }),
    ...
    {
      pattern: /permanent/,
      handler: () => ({ number: 0, scale: "permanent" }),
    },
  ];
